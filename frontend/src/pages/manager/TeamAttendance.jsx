...
import { Calendar, Search, Filter, RefreshCw, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
const TeamAttendance = () => {
  ...
  const fetchTodayLogs = async () => {
    ...
    try {
      const response = await axios.get('/api/attendance/team-today');
      setTodayLogs(response.data);
    } catch (err) {
      console.error('Error fetching today team attendance:', err);
    } finally {
  ...
  const handleFetchHistory = async (e) => {
    ...
    try {
      const response = await axios.get(`/api/attendance/team-history?startDate=${startDate}&endDate=${endDate}`);
      setHistoryLogs(response.data);
    } catch (err) {
      console.error('Error fetching history team attendance:', err);
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Supervise and audit attendance history for your team.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* Today's Logins */}
        <div className="glass bg-white dark:bg-slate-900 border dark:border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col h-[600px]">
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-3.5">
              ...
              {todayLogs.length === 0 && (
                <div className="text-center py-12 text-slate-400 dark:text-slate-500 italic text-sm">
                  No check-ins recorded for your team today yet.
                </div>
              )}
            </div>
          )}
        ...
        <div className="glass bg-white dark:bg-slate-900 border dark:border-slate-800 p-6 rounded-2xl shadow-sm lg:col-span-2 flex flex-col h-[600px]">
          <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2 text-md">
            <Calendar size={18} className="text-brand-500" />
            Team Logs History
          </h3>

          <form onSubmit={handleFetchHistory} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end mb-6 bg-slate-50 dark:bg-slate-800/20 p-4 rounded-xl border dark:border-slate-850 shrink-0">
            <div>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {loadingHistory ? (
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="grid grid-cols-4 gap-4 p-4 mb-4 bg-brand-50/50 dark:bg-brand-950/10 border border-brand-100/50 dark:border-brand-900/20 rounded-xl text-center shrink-0">
                  <div className="space-y-0.5">
                    <span className="text-[10px] text-slate-500 uppercase font-medium">Logged Days</span>
                  ...
                  <div className="space-y-0.5">
                    <span className="text-[10px] text-brand-650 uppercase font-medium">Presence %</span>
                    <p className="text-lg font-bold text-brand-600 dark:text-brand-450">{metrics.percentage.toFixed(0)}%</p>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 text-sm">
                ...
                Select date ranges above to audit team logs history.
              </div>
            )}
          </div>
        </div>
